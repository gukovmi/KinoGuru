...
class GetGoogleStandardSignInIntentUseCaseTest {

    private val googleSignInIntentRepository: GoogleSignInIntentRepository = mock()
    private val useCase = GetGoogleStandardSignInIntentUseCase(googleSignInIntentRepository)

    @Test
    fun `invoke EXPECT sign in intent`() = runTest {
        val signInIntent: PendingIntent = mock()
        whenever(googleSignInIntentRepository.get(GoogleAuthVariant.STANDARD)).thenReturn(signInIntent)
